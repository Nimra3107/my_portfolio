      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.nav
            ...
            exit={{ opacity: 0, y: -10 }}
            className="md:hidden absolute top-full left-0 right-0 bg-slate-900 border-b border-slate-800 p-4 sm:p-6 flex flex-col gap-2 shadow-xl max-h-[calc(100svh-4rem)] overflow-y-auto"
          >
            {NAV_LINKS.map((link) => (
              <a
                key={link.name}

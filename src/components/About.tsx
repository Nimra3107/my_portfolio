...
export default function About() {
  return (
    <section id="about" className="py-16 sm:py-20 lg:py-24 bg-slate-900/50">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
          ...
          <motion.div
            ...
            className="bg-slate-800/50 rounded-2xl p-5 sm:p-8 border border-slate-700"
          >
...
function InfoItem({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <li className="flex items-center gap-4">
      <div className="w-12 h-12 shrink-0 rounded-full bg-slate-900 flex items-center justify-center border border-slate-700">
        {icon}
      </div>
      <div className="min-w-0">
        <span className="block text-sm text-slate-400 font-medium">{label}</span>
        <span className="block text-white font-medium text-sm min-[380px]:text-base break-words">{value}</span>
      </div>
    </li>
  );
}
